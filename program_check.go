		...
		if x1, y := ctx.defs["TYPE"]; y {
			if x2, y := ctx.defs["TARGET"]; y {
				...
				if t := __string(ctx, x2); "SIZEOF_"+s != t && "ALIGNOF_"+s != t {
					erro(ctx, "%v : %s != %s", ctx.prerequisite, s, t)
				}
				...
				if x, y := dep.(*argumented); y && x.Value.String() == "$(name).c.x" {
					...
					for i, s := range []string{"$(TYPE)","$(INCLUDE)","$(LIB)"} {
						if x.args[i].String() != s {
							erro(ctx, "%v %v %v != %s", t, x.Value, x.args[i], s)
						}
					}
				}
			}
...
func (prog *program) execute_check_rule_0(ctx Context, result *Value) {
    ...
    case "rule0":
        if len(args) != 3 {
            erro(ctx, "%v: %d %v", ent, len(args), ts(args))
        }
		...
		if auto_get(ctx, "<") != auto_get(ctx, ">") {
			erro(ctx, "%v %v", ts(auto_get(ctx, "<")), ts(auto_get(ctx, ">")))
		}
        if x, y := (*result).(*list); !y {
            erro(ctx, "%v: %v", ent, ts(*result))
        } else if x.len() != 4 {
            erro(ctx, "%v: %v", ent, ts(*result))
        ...
        if ts(*result) != "{=list {=word rule1} {=word rule1} {=flag {=null}} {=compound {=word x} {=word y} {=word z}}}" {
            erro(ctx, "%v: %v, %v", ent, ts(*result), args)
        }
    case "rule1":
		if len(args) != 1 {
			erro(ctx, "%v: %d %v", ent, len(args), ts(args))
		...
		if v := auto_get(ctx, "ARG1"); v == nil {
			...
			if s, t := __string(ctx, *result), "xxyzz\n"; s != t {
				erro(ctx, "%v: %v: %s != %s", ent, ts((*result)), s, t)
			}
			...
			erro(ctx, "%v : %s != %s ; %s", v, s, t, __string(ctx, v))
		}
    }
}
...
func (prog *program) execute_check_shell_for_stdout(ctx Context, result *Value) {
    ...
    case ".test.0":
		if len(_execution(ctx).interpreted) == 0 {
            erro(ctx, "%v: not interpreted", ent)
		}
        if len(args) != 2 {
            erro(ctx, "%v: %d %v", ent, len(args), ts(args))
        }
        if len(prog.params) != 2 {
            erro(ctx, "%v: %d %v", ent, len(prog.params), ts(prog.params))
        }
		...
		if v := auto_get(ctx, "-"); ts(v) != ts(*result) {
			errostack(ctx, 3, "%s != %s", ts(v), ts(*result))
		}
		...
		case defExpand0:
			if ts(*result) != sf("{=delegate {=builtin debug} {=list %s %s}}", ts(args[1]), ts(args[0])) {
				errostack(ctx, 3, "%v (%s %s)", ts(*result), ts(args[1]), ts(args[0]))
			}
		case defExpand1:
			if ts(*result) != "{=null}" {
				errostack(ctx, 3, "%v (%s %s)", ts(*result), ts(args[1]), ts(args[0]))
		...
		case "{=[Value] {=list {=delegate {=auto 1}}} {=list {=delegate {=auto 2}}}}":
			...
			case defExpand0, defExpand1:
				if ts(*result) != "{=delegate {=builtin debug} {=list {=delegate {=auto 2}} {=delegate {=auto 1}}}}" {
					errostack(ctx, 3, ".test: %v, %s", ts(*result), ts(args))
				}
			}
		default:
			switch o {
			case defExpand0:
				...
				if ts(*result) != t {
					errostack(ctx, 3, ".test: %v != %s, %s", ts(*result), t, ts(args))
				}
			case defExpand1:
				if ts(*result) != "{=null}" {
					errostack(ctx, 3, ".test: %v %v", ts(*result), ts(args))
				}
			}
		}
	}
	...
	default:
		errostack(ctx, 5, "untested: %v: %v %s %s", ent.destiny(), o, ts(args), ts(*result))
	}
}
