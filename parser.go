...
func (l ul) closuredelegate_obj(ctx Context, isClosure bool, tok token, name Value) (obj Value, str string, opts []Value) {
	...
	if v := name.expand(c); c.i == ident_dis || isNull(v) {
...
func (l ul) abc(ctx Context) (tok token, obj Value, args, opts []Value) {
	...
	case LPAREN, LBRACE: // $(...), ${...}
		...
		if (tok == LPAREN && l.p.tok != RPAREN) || (tok == LBRACE && l.p.tok != RBRACE) {
			...
			switch str {
			case "":
				note(pc(cc,name), "%v → %v", name, obj).debug()
				l.p.spaces(cc)
...
func (l ul) unary(ctx Context) (x Value) {
	...
	default:
		if t := l.p.tok; (CLOSURE <= t && t <= CLOSURE__) || (DELEGATE <= t && t <= DELEGATE__) {
			return l.closuredelegate(ctx)
		} else if t.is_keyword() { // keywords here are words
			return l.p.bare(ctx)
		}
	}
            ...
            if a || neg {
                if n := len(capture); n == 0 {
					...
					for i, cap := range c {
						...
						if ac != nil { ac.set(ctx, defAuto, s, w) }
					}
					vals = append(vals, dc)
					break
                } else if n <= len(c) {
					...
					for i, cap := range capture {
						...
						if ac != nil { ac.set(ctx, defAuto, cap, w) }
					}
					vals = append(vals, dc)
					break
				}
				...
				if !isTrivial(val) {
					if x, y := val.(*defscapture); y {
						...
						for _, c := range x.caps {
							ac.set(&ac, defAuto, c.name, c.val)
						}
					} else {
						ac.set(&ac, defAuto, "_", val)
					}
				...
				for _i, _v := range params {
					...
					for _, v := range _v.a {
						if i < len(v.elems) {
							...
							ac.set(&ac, defAuto, v.name, &null{valbase{_v.p}})
						}
					}
				}
