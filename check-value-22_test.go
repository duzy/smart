...
func testValues22(ctx *testcase) {
	...
	} else if s, t := v.string(src(ctx,d)), "1 10 2 20 3 foo_ba-- 30 4 40"; s != t {
		ctx.err("%s: %s != %s %s", d.name, s, t, tst{v})
	...
	if d == nil {
		...
		ctx.err("%s: %s != %s %s", d.name, s, t, tst{v})
	} else if s, t := v.string(src(ctx,d)), "1 10 2 20 3 foo_ba-- 30"; s != t {
		ctx.err("%s: %s != %s %s", d.name, s, t, tst{v})
	}
	...
	if d == nil {
		...
		ctx.err("%s != %s | %v | %s", s, t, v, tst{v})
	} else if s, t := v.string(src(ctx,d)), "1 10 2 20 3 foo_ba-- 30 4 40 . x"; s != t {
		ctx.err("%s != %s | %v | %s", s, t, v, tst{v})
