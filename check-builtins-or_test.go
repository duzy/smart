...
func test__or(ctx *testcase) {
	if v := ctx.val("val11.0"); v == nil {
		...
		ctx.err("%v", tst{v})
	} else if s := v.string(src(ctx,nil)); s != "-no -yes -false -true" {
		ctx.err("%v ⇒ %s", tst{v}, s)
	...
	if v := ctx.val("val11"); v == nil {
		...
		ctx.err("%v", tst{v})
	} else if s := v.string(src(ctx,nil)); s != "-no" {
		ctx.err("%v ⇒ %s", tst{v}, s)
	}
	...
	if v := ctx.val("val12"); v == nil {
		...
		ctx.err("%v", tst{v})
	} else if s := v.string(src(ctx,nil)); s != "-yes" {
		ctx.err("%v ⇒ %s", tst{v}, s)
	}
	...
	if v := ctx.val("val13"); v == nil {
		...
		ctx.err("%v", tst{v})
	} else if s := v.string(src(ctx,nil)); s != "-false" {
		ctx.err("%v ⇒ %s", tst{v}, s)
	}
}
