...
func TestCheckCheckFiles(t *testing.T) {
        if _, err := os.Stat("example"); err != nil { t.Error(err) }
        if _, err := os.Stat("example.o"); err != nil { t.Error(err) }
        if _, err := os.Stat("example.cpp"); err != nil { t.Error(err) }
}
...
`))
                        ...
                        v = []byte(fmt.Sprintf(`update file 'example.cpp' ... (ok)
g++ -std=c++1z -c example.cpp -o example.o
g++ -std=c++1z example.o -o example -lpthread 
./example
Hello World!
`))
